...
EPUBReader = function(epubpath) {
    ...
    // Load contents file
    tocPath = $('spine', this.$content).attr("toc");
    tocPath = this.dir + "/" + $('#' + tocPath, this.$content).attr('href');    
    temp = fs.readFileSync(tocPath, 'utf8');
    this.$contents = $(temp);
    
    // Create local book path for assets
    this.filename = path.basename(epubpath);
    temp = this.dir.split(path.sep);
    temp = _.rest(temp, temp.lastIndexOf(this.filename));
    this.local = temp.join(path.sep);
    this.local += "/";
};
...
_.extend(EPUBReader.prototype, {
    ...
    cover : function() {
        ...
        return this.local + $('#cover', this.$content).attr('href');
    },
    ...
    asset : function(id) {
        ...
        return this.local + this.manifest[id];  
    },
    ...
    toc : function() {
        ...
        navPoints.each(function(index, el){
            ...
            contents.push(objectForNavPoint(el, local));
        });
...
function objectForNavPoint(navPoint, prepend) {
    ...
    obj.id = $nav.attr("id");
    obj.src = prepend + $nav.children('content').attr("src");
    ...
    $children.each(function(index, el){
        ...
        obj.children.push(objectForNavPoint(el, prepend));
    });
